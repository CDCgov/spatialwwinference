---
title: "Getting started with spatialwwinference"
description: "A quick start example demonstrating the use of spatialwwinference to jointly fit wastewater and hospital admissions data"
...
pkgdown:
  as_is: true
vignette: >
  %\VignetteIndexEntry{Getting started with spatialwwinference}
...
In this quick start, we demonstrate using `spatialswwinference` to specify and fit a
...
interested in fitting the `spatialwwinference` model to their own data.

# Package

In this quick start, we also use `dplyr` `tidybayes` and `ggplot2` packages.
These are installed as dependencies when `spatialwwinference` is installed.
...
These data are already in a format that can be used for `spatialwwinference`. For the
...
```{r}
hosp_data <- spatialwwinference::hosp_data
hosp_data_eval <- spatialwwinference::hosp_data_eval
ww_data <- spatialwwinference::ww_data
...
```{r}
ww_data_preprocessed <- spatialwwinference::preprocess_ww_data(
  ww_data,
...
fitting functions.
```{r}
hosp_data_preprocessed <- spatialwwinference::preprocess_hosp_data(
  hosp_data,
...
```{r}
ww_data_to_fit <- spatialwwinference::indicate_ww_exclusions(
...
inf_to_hosp <- spatialwwinference::inf_to_hosp

# Assign infection feedback equal to the generation interval
infection_feedback_pmf <- generation_interval
```
We will pass these to the `model_spec()` function of the `spatialwwinference()` model,
along with the other specified parameters above.


# Precompiling the model
As `spatialwwinference` uses `cmdstan` to fit its models, it is necessary to first
compile the model. This can be done using the compile_model() function.
```{r}
model <- spatialwwinference::compile_model()
```
# Fitting the model
...
pre-compiled model(`model`) to `spatialwwinference()` where they are combined and
used to fit the model.

```{r, message = FALSE}
fit <- spatialwwinference::spatialwwinference(
  ww_data_to_fit,
...
# The `spatialwwinference` object
The `spatialwwinference()` function returns a `spatialwwinference` object which includes
...
## Diagnostics
While the `spatialwwinference()` function will print out messaging if any of the
...
```{r}
convergence_flag_df <- spatialwwinference::get_model_diagnostic_flags(
  stan_fit_obj =
    fit$raw_fit_obj
)
